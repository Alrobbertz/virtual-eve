...
class AugResnet2(nn.Module):
    def __init__(self, resnet, outSize, dropout):
        ...
        self.resnet_18.conv1 = nn.Conv2d(
        ...
        if dropout:
            self.fc = nn.Sequential(nn.Dropout(0.5), nn.Linear(512 + 9, outSize))
        else:
...
class AugResnet(nn.Module):
    def __init__(self, resnet, outSize, dropout):
        ...
        self.mlp = nn.Sequential(nn.Linear(9, 1024), nn.ReLU(), nn.Linear(1024, 512))

        if dropout:
            self.fc = nn.Sequential(nn.Dropout(0.5), nn.Linear(512 + 512, outSize))
        else:
...
class AugmentedCNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc = nn.Sequential(
            nn.Linear(16 + 9, 100), nn.LeakyReLU(), nn.Linear(100, 14)
        )
    ...
    def __init__(self, inplanes, hiddenplanes, outplanes, dropout=False):
        ...
        if dropout:
            self.model = nn.Sequential(
                nn.Linear(inplanes, hiddenplanes),
                nn.Dropout(p=0.5),
                nn.ReLU(),
                nn.Linear(hiddenplanes, outplanes),
            )
        else:
            self.model = nn.Sequential(
                nn.Linear(inplanes, hiddenplanes),
    ...
    def __init__(self, inplanes, hiddenplanes, outplanes, dropout=False):
        ...
        if dropout:
            self.model = nn.Sequential(
                nn.Linear(inplanes, hiddenplanes),
                nn.ReLU(),
                nn.Linear(hiddenplanes, hiddenplanes),
                nn.ReLU(),
                nn.Linear(hiddenplanes, hiddenplanes),
                nn.Dropout(p=0.5),
                nn.ReLU(),
...
class ChoppedAlexnetBN(nn.Module):
    ...
    def __init__(self, numLayers, outSize, dropout):
        ...
        if dropout:
            self.model = nn.Sequential(
                nn.Dropout(p=0.5), nn.Linear(channelSize, outSize)
            )
        else:
...
class ChoppedAlexnetSpatial(nn.Module):
    ...
    def __init__(self, numLayers, outSize, returnIntermediate=True):
        ...
        self.features = nn.Sequential(
            *(layers + [nn.Conv2d(channelSize, outSize, kernel_size=1)])
        )
        self.pool = nn.AdaptiveAvgPool2d((1, 1))
