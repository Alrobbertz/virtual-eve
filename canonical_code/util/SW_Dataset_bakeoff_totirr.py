...
def eve_scale(
    eve_sample, eve_means, eve_stds, scale="sqrt", eve_sigmoid=False, zscore=True
):
...
class SW_Dataset(Dataset):
    ...
    def __init__(
        ...
        self.index_aia = AIA_root + np.asarray(
            df_indices[[channel for channel in df_indices.columns[2:-1]]]
        )
        ...
        if EVE_sigmoid:
            self.EVE_means = np.load(
                index_file + "eve_" + EVE_scale + "sigmoid" + "_mean.npy"
            )
            self.EVE_stds = np.load(
                index_file + "eve_" + EVE_scale + "sigmoid" + "_std.npy"
            )
        ...
        if self.crop and self.crop_res > self.resolution:
            raise ValueError(
                "Cropping resolution must be smaller than initial resolution"
            )
    ...
    def __getitem__(self, index):
        ...
        AIA_sample = np.asarray(
            [
                ...
                for channel in self.index_aia[index, :]
            ],
            dtype=np.float32,
        )
                ...
                [
                    np.expand_dims(
                        ...
                        * skimage.transform.downscale_local_mean(
                            AIA_sample[i, :, :], (divide, divide)
                        ),
                        axis=0,
                    )
                    for i in range(AIA_sample.shape[0])
                ]
            ),
            dtype=np.float32,
        )
        ...
        if self.crop:
            hcrop_start = np.random.randint(
                0, high=(self.resolution - self.crop_res) + 1
            )
            hcrop_end = hcrop_start + self.crop_res

            vcrop_start = np.random.randint(
                0, high=(self.resolution - self.crop_res) + 1
            )
        ...
        if self.flip:
            ...
            AIA_sample = torch.from_numpy(
                aia_scale(AIA_sample_temp.copy(), self.zscore, self.self_mean_normalize)
            )
        else:
            AIA_sample = torch.from_numpy(
                aia_scale(AIA_sample, self.zscore, self.self_mean_normalize)
            )

        EVE_sample = torch.from_numpy(
            eve_scale(
                self.EVE[index, :],
                self.EVE_means,
                self.EVE_stds,
