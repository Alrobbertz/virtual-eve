...
def train_model(
    model, dataloaders, device, criterion, optimizer, scheduler, num_epochs
):
...
if __name__ == "__main__":
    ...
    for cfgi, cfgPath in enumerate(cfgs):
        ...
        if cfg["arch"] == "resnet_18":
            ...
            sw_net.conv1 = nn.Conv2d(
                9, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False
            )
            if cfg["dropout"]:
                sw_net.fc = nn.Sequential(nn.Dropout(0.5), nn.Linear(num_ftrs, 15))
            else:
            ...
            sw_net.layer0[0] = nn.Conv2d(
                9, 16, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), bias=False
            )
            if cfg["dropout"]:
            ...
            sw_net = alt_models.AugResnet2(
                models.resnet18(pretrained=False), 15, cfg["dropout"]
            )
        ...
        sw_net, train_loss, val_loss = train_model(
            sw_net,
            sw_dataloaders,
            "cuda",
            criterion,
