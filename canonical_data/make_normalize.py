...
if __name__ == "__main__":
    ...
    eve_all = np.load(args.irradiancefile)[
    ...
    index_aia = args.base + np.asarray(
        indices[[channel for channel in indices.columns[2:11]]]
    )
    ...
    for index in range(0, Y.shape[0], 20):
        ...
        X = np.asarray(
            [
                np.load(channel.replace(".fits", ""))["x"]
                for channel in index_aia[index, :]
            ]
